...
" Run the following when vim starts.
...
" Check if NERDTree is open or active, sync open file with NERDTree
...
noremap <silent> <expr> k (v:count == 0 ? 'gk' : 'k') " j/k will move virtual lines (lines that wrap)
...
nnoremap <C-p> :Files<Cr>  " remap Ctrl-p for finding files run Fzf :Files command
...
let g:coc_global_extensions = [
  ...
  \ 'coc-json', 
  \ ]
" from readme if hidden is not set, TextEdit might fail.
